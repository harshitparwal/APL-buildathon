...
function handleSyncEvent(data) {
    ...
    if (type === 'match_reset') {
        ...
        // Auto resolve after some time to ensure the user gets a result
        setTimeout(() => {
        ...
        if (voteFinalVerdict) {
            ...
            if (currentVote) {
                if (currentVote === actualResult) {
                    msg += `<div style="color: var(--success); font-weight: bold;">✅ Good call!</div>`;
                } else {
...
function startPredictionTimer() {
    ...
    predInterval = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            predInterval = null;
            predictionActive = false;
            predTimer.textContent = "Predictions locked!";
...
function scheduleNextBall(delayMs = null) {
    ...
    autoSimTimeout = setTimeout(() => {
        ...
        // 10s prediction timer + 2s to resolve
        setTimeout(() => {
            if (!autoSimActive || !matchActive) return;
            const results = ['dot', 'single', 'boundary', 'wicket'];
            const randomResult = results[Math.floor(Math.random() * results.length)];
            ...
            if (randomResult === 'wicket' && Math.random() < 0.5) {
                ...
                setTimeout(() => {
                    ...
                    // Review takes 10s to resolve + 8s to hide = 18s
                    scheduleNextBall(18000 + (Math.random() * 15000 + 10000));
                }, 3000);
            } else if (randomResult === 'boundary') {
                setTimeout(() => {
                    if (!autoSimActive || !matchActive) return;
                    broadcastEvent('admin_big_moment', { runs: exactRuns });
                    // Big moment chat is open for 30s
                    scheduleNextBall(30000 + (Math.random() * 15000 + 10000));
                }, 2000);
            } else {
                scheduleNextBall(); // Normal delay 10-30s
            }
